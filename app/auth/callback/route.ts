...
async function findAndConfirmPendingTicket(user: any, request: NextRequest) {
  try {
    const adminSupabase = createAdminClient()
    ...
    if (pendingTicket) {
      return await confirmPendingTicket(pendingTicket, request)
    }
  } catch (confirmError) {
...
async function handleSuccessfulAuth(
  ...
  const redirectResponse = await findAndConfirmPendingTicket(user, request)
