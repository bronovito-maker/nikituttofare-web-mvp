...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50/50">
      ...
      {/* Header Minimal */}
      <header className="sticky top-0 z-50 w-full border-b border-slate-200/30 bg-white/75 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-emerald-700 bg-emerald-50 px-3 sm:px-4 py-1.5 rounded-full border border-emerald-200/50">
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12 sm:py-16 relative">
        ...
        <div className="relative z-10 w-full max-w-md">
          ...
          <ClientAnimationWrapper delay={0.1} duration={0.6}>
            <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl shadow-slate-200/50 border border-slate-200/50 overflow-hidden">
              
              {/* Header Card */}
              <div className="px-6 sm:px-8 pt-8 pb-6 text-center space-y-3 bg-gradient-to-b from-slate-50/50 to-transparent">
              ...
              <div className="px-6 sm:px-8 pb-8 pt-2">
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <Button
                    type="submit"
                    disabled={isLoading}
                    className="w-full h-12 sm:h-14 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-base shadow-lg shadow-slate-200/50 transition-all hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed group"
                  >
                    {isLoading ? (
                  ...
                  {emailSent && (
                    <div className="p-4 rounded-xl bg-green-50 border border-green-200 text-green-800 text-sm flex items-start gap-2">
                      <CheckCircle2 className="w-5 h-5 flex-shrink-0 mt-0.5" />
                      <div>
                        <p className="font-semibold">Email inviata!</p>
                        <p className="text-green-700">Controlla la casella di posta e clicca sul link per accedere.</p>
                      </div>
                    </div>
                  )}
                </form>
      ...
      {/* Footer Minimal */}
      <footer className="py-6 text-center border-t border-slate-200/30 bg-white/50 backdrop-blur-sm">
        <p className="text-xs text-slate-500">
          © 2024 NikiTuttoFare • <Link href="/" className="hover:text-slate-700 transition-colors">Home</Link> • Privacy • Termini
        </p>
      </footer>
    </div>
  );
